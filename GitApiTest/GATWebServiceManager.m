    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        ...
        [session dataTaskWithRequest:requestData completionHandler:^(NSData *data,
            ...
            if (!error && statusRes == 200) {
                ...
                //SaveData
                strongSelf.credential = [[UrlCredentials alloc] init];
                [strongSelf.credential addNewData:[dict valueForKey:@"scope"]
                                 andWithTokenPass:[dict valueForKey:@"access_token"]];
                [strongSelf.loginVC pushTableRepos];
            } else {
                NSLog(@"%@", error);
            }
        }];
        ...
        [session dataTaskWithRequest:
         requestData completionHandler: ^(NSData *data,NSURLResponse *response,NSError *error) {
             ...
             if (!error && statusRes == 200) {
                 ...
                 [strongSelf getAdditionalUserInfo:viewController withArr:userRepoArray];
             }
             else {
                 NSLog(@"%@", error);
             }
